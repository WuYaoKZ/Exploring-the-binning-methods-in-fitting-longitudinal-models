...
grid <- seq(0, 1, length = L)  # L length sequence from 0 to 1
beta_true <- matrix(NA, 2, L)  # matrix made of NA's, 2 rows, L cols
scenario <- 1 ## indicate the scenario to use
if(scenario == 1){
  beta_true[1,] = -0.15 - 0.1*sin(2*grid*pi) - 0.1*cos(2*grid*pi)  # each row: L length list
...
for(iter in 1:nsim){
  ...
  ptm <- proc.time()  # how much cpu time
  ...
  ################################################################################
  ## lfosr3s
  MISE_lfosr3s <- rowMeans((fit_lfosr3s$betaHat - beta_true)^2)  # mean integrated squared error 
  cover_lfosr3s <- rep(NA, nrow(beta_true))  # length=2
  ...
  for(p in 1:length(cover_lfosr3s)){
    ...
    cover_lfosr3s[p] <- length(intersect(cover_lower, cover_upper)) == L  # check if CI cover the true value: joint
    ...
    cover_lfosr3s_pw[p,intersect(cover_lower_pw, cover_upper_pw)] <- TRUE  # check if CI cover the true value: point-wise
  }
...
################################################################################
## MISE
MISE_lfosr3s <- lapply(sim_local, '[[', 1) %>% bind_rows()  # extract the first element, and bind by row
MISE_pffr <- lapply(sim_local, '[[', 2) %>% bind_rows()  # lappy() return a list of same length of sim_local (nsim)
...
## coverage
### joint
cover_lfosr3s <- t(lapply(sim_local, '[[', 3) %>% bind_cols())  # t() transpose the results
